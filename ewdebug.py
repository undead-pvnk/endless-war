...
async def debug8(cmd):
	...
	if user_data.life_state == ewcfg.life_state_executive or author.server_permissions.administrator:
...
async def scrutinize(cmd):
	...
	if user_data.poi not in debugroom_set:
		...
		if searched_object == "personalcomputer" and found_object == True:
			...
			try:
				msg = await cmd.client.wait_for_message(timeout=30, author=member)
