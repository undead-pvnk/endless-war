    ...
    while True:
        ...
        if False:
            ...
            try:
                ...
                json_string = ""
                p = subprocess.Popen(
                    "curl -H 'Client-ID: {}' -X GET 'https://api.twitch.tv/helix/streams?user_login = rowdyfrickerscopkillers' 2>/dev/null".format(
                        twitch_client_id),
                    shell=True,
        ...
        try:
            for msg_file in os.listdir(ewcfg.dir_msgqueue):
                ...
                if len(msg_channel_names) == 0:
                    ewutils.logMsg(
                        'in file {} message for channel {} (reverb {})\n{}'.format(msg_file, msg.channel, msg.reverb,
                                                                                   msg.message))
                else:
                    ...
                    for server in client.servers:
                        for channel in server.channels:
                            if channel.name in msg_channel_names:
                                ...
                                await ewutils.send_message(client, channel,
...
async def on_message(message):
    ...
    if message.server != None:
        try:
            ewutils.execute_sql_query(
                "UPDATE users SET {time_last_action} = %s WHERE id_user = %s AND id_server = %s".format(
            ...
            ewutils.logMsg(
                'server {}: failed to update time_last_action for {}'.format(message.server.id, message.author.id))
    ...
    if message.content.startswith(ewcfg.cmd_prefix) or message.server == None or len(message.author.roles) < 2:
        ...
        try:
            ...
            return await ewutils.send_message(client, message.channel, ewutils.formatMessage(message.author,
                                                                                             "We don't take kindly to moon runes around here."))
    ...
    elif content_tolower.find(ewcfg.cmd_howl) >= 0 or content_tolower.find(ewcfg.cmd_howl_alt1) >= 0 or re_awoo.match(
            content_tolower):
