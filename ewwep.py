	...
	if ewmap.channel_name_is_poi(cmd.message.channel.name) == False:
		...
		if search != None:
			...
			if shootee_data.life_state == ewcfg.life_state_kingpin:
				...
				response = "You don't have enough slime to attack. ({:,}/{:,})".format(user_data.slimes,
																					   slimes_spent)
				...
				user_data.change_slimes(
					n=(-user_data.slimes if slimes_spent >= user_data.slimes else -slimes_spent),
				...
				await
				ewrolemgr.updateRoles(client=cmd.client, member=cmd.message.server.get_member(shootee_data.id_user))
				...
				if was_shot:
					...
					if was_killed:
						...
						ewstats.track_maximum(user=user_data, metric=ewcfg.stat_biggest_kill,
						...
						deathreport = "You were {} by {}. {}".format(kill_descriptor,
																	 cmd.message.author.display_name,
																	 ewcfg.emote_slimeskull)
						deathreport = "{} ".format(ewcfg.emote_slimeskull) + ewutils.formatMessage(member,
																								   deathreport)
					...
					# response += explode_resp
						...
						if weapon != None:
							...
							if miss:
								response = "{target_name} dodges your strike.".format(
									target_name=member.display_name)
							else:
				...
				if was_killed:
					await
					ewrolemgr.updateRoles(client=cmd.client, member=member)


			response = "**DEBUG**: You hunt {}. Nice work!".format(search.name)
			ewhunting.delete_enemy(search)
			print(search.id_enemy)

			# if search.type == "110k slime":

			# elif search.type == "2 poudrin dropper":
