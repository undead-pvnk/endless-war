	...
	if response == "":
		...
		if shootee_data.life_state == ewcfg.life_state_corpse:
			...
			if was_shot:
				...
				user_data.time_expirpvp = ewutils.calculatePvpTimer(user_data.time_expirpvp, (int(time.time()) + ewcfg.time_pvp_attack), True)
			...
			if was_killed:
				...
				# Flag the user for PvP
				user_data.time_expirpvp = ewutils.calculatePvpTimer(user_data.time_expirpvp, (int(time.time()) + ewcfg.time_pvp_kill), True)
		...
		if(member.id == cmd.message.author.id):
			...
			if user_data.hunger >= ewutils.hunger_max_bylevel(user_data.slimelevel):
				...
				if was_sparred:
					...
					# Flag the user for PvP
					user_data.time_expirpvp = ewutils.calculatePvpTimer(user_data.time_expirpvp, (int(time.time()) + ewcfg.time_pvp_spar), True)
