...
def canAttack(cmd, amb_switch = 0):
	...
	#	response = "You don't have enough sap to attack. ({}/{})".format(user_data.sap, weapon.sap_cost)
		...
		elif not poi.pvp and not (shootee_data.life_state == ewcfg.life_state_shambler or shootee_data.get_inhabitee() == user_data.id_user or user_isshambler) or (shootee_data.life_state == ewcfg.life_state_juvenile and shootee_data.slimes <= ewcfg.max_safe_slime):
			# Target is neither flagged for PvP, nor a shambler, nor a ghost inhabiting the player, nor a juvie above 10k slime. Player is not a shambler.
			response = "{} is not mired in the ENDLESS WAR right now.".format(member.display_name)
