    ...
    if ewmap.channel_name_is_poi(cmd.message.channel.name) == False:
        ...
        if shootee_data.life_state == ewcfg.life_state_kingpin:
            ...
            if was_busted:
                ...
                ewstats.track_maximum(user=user_data, metric=ewcfg.stat_biggest_bust_level,
                ...
                if coinbounty > 0:
                    response += "\n\n SlimeCorp transfers {} SlimeCoin to {}\'s account.".format(str(coinbounty),
            ...
            if shootee_data.life_state in [ewcfg.life_state_enlisted, ewcfg.life_state_juvenile,
...
async def suicide(cmd):
    ...
    if ewmap.channel_name_is_poi(cmd.message.channel.name) == False:
        ...
        if user_isdead:
            ...
            response = '{} has willingly returned to the slime. {}'.format(cmd.message.author.display_name,
                                                                           ewcfg.emote_slimeskull)
    ...
    if cmd.message.channel.name != ewcfg.channel_dojo:
        ...
        if (member.id == cmd.message.author.id):
            ...
            if user_data.hunger >= ewutils.hunger_max_bylevel(user_data.slimelevel):
                ...
                weapon = None
                if user_data.weapon != "" and sparred_data.weapon != "" and weapon_item.item_props.get(
                        "weapon_type") == sparred_weapon_item.item_props.get("weapon_type"):
                    weapon = ewcfg.weapon_map.get(weapon_item.item_props.get("weapon_type"))
                    duel = True
                ...
                if sparred_data.life_state == ewcfg.life_state_corpse:
                    ...
                    was_juvenile = True
                        ...
                        sparred_data.life_state == ewcfg.life_state_enlisted and sparred_data.faction == ewcfg.faction_killers)) or (
                        user_isrowdys and (
                        sparred_data.life_state == ewcfg.life_state_enlisted and sparred_data.faction == ewcfg.faction_rowdys)):
                    # User can be sparred.
                    was_sparred = True
                elif (user_iskillers and (
                        sparred_data.life_state == ewcfg.life_state_enlisted and sparred_data.faction == ewcfg.faction_rowdys)) or (
                        user_isrowdys and (
                        sparred_data.life_state == ewcfg.life_state_enlisted and sparred_data.faction == ewcfg.faction_killers)):
                ...
                if was_sparred:
                    ...
                    if duel:
                        ...
                        if weaker_player.weaponskill < 5 or (
                                weaker_player.weaponskill + 1) < stronger_player.weaponskill:
                            weaker_player.add_weaponskill(n=1, weapon_type=weapon.id_weapon)

                        if stronger_player.weaponskill < 5 or (
                                stronger_player.weaponskill + 1) < weaker_player.weaponskill:
                    ...
                    if duel and weapon != None:
                        response = weapon.str_duel.format(name_player=cmd.message.author.display_name,
                                                          name_target=member.display_name)
                    else:
                    ...
                    if weapon != None:
                        if user_data.weaponskill >= 5:
                            response += ' {} is a master of the {}.'.format(cmd.message.author.display_name,
                                                                            weapon.id_weapon)
    ...
    if cmd.message.channel.name != ewcfg.channel_dojo:
        ...
        if weapon != None:
            if weapon.id_weapon != 'gun' and ewcfg.weapon_fee > user_data.slimecoin:
                response = "The fee for taking a weapon is {} slimecoin and you only have {}.".format(ewcfg.weapon_fee,
                                                                                                      user_data.slimecoin)
            ...
            response = "Choose your weapon: {}".format(
                ewutils.formatNiceList(names=ewcfg.weapon_names, conjunction="or"))
...
async def marry(cmd):
    ...
    weapon_name = weapon_item.item_props.get("weapon_name") if len(
        weapon_item.item_props.get("weapon_name")) > 0 else weapon.str_weapon
    ...
    # Makes sure you have a displayed rank 6 or higher weapon.
        ...
        response = "Slow down, Casanova. You do not nearly have a close enough bond with your {} to engage in holy matrimony with it. You’ll need to reach rank 8 mastery or higher to get married.".format(
            weapon_name)
        await ewutils.send_message(cmd.client, cmd.message.channel, ewutils.formatMessage(cmd.message.author, response))
...
async def divorce(cmd):
    ...
    if weapon != None:
        ...
        # Makes sure you have a partner to divorce.
            ...
            user_data.weapon = -1
            ewutils.weaponskills_set(member=cmd.message.author, weapon=weapon_item.item_props.get("weapon_type"),
                                     weaponskill=0)
