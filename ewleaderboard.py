...
def make_slimernalia_board(server, title):
	...
	data = ewutils.execute_sql_query(
		"SELECT {display_name}, {state}, {faction}, {festivity} + COALESCE(sigillaria, 0) + FLOOR({coin_gambled} / 1000000000000) as total_festivity FROM users LEFT JOIN (SELECT id_user, COUNT(*) * 100 as sigillaria FROM items INNER JOIN items_prop ON items.{id_item} = items_prop.{id_item} WHERE {name} = 'id_cosmetic' AND {value} = %s GROUP BY items.{id_user}) f on users.{id_user} = f.{id_user}, players WHERE users.{id_server} = %s AND users.{id_user} = players.{id_user} ORDER BY {festivity} DESC LIMIT 5".format(
			id_user = ewcfg.col_id_user,
