...
async def recreateRoles(cmd):
    ...
    server_role_names = {}
    for role in server.roles:
        server_role_names[ewutils.mapRoleName(role.name)] = role.id
    ...
    for poi in ewcfg.poi_list:
         ...
         if poi.role != None:
            ...
            if poi.role not in list(server_role_names.keys()):
                ...
                newrole = server.get_role(server_role_names.get(poi.role))
            channel_obj = ewutils.get_channel(cmd.message.guild, poi.channel)

            if channel_obj:
                await channel_obj.set_permissions(newrole, read_messages = True, send_messages = True)