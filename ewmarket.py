...
class EwStock:
        ...
        if id_server is not None and stock is not None:
            ...
            if timestamp is not None:
                data = ewutils.execute_sql_query(
                    "SELECT {stock}, {market_rate}, {exchange_rate}, {boombust}, {total_shares}, {timestamp} FROM stocks WHERE id_server = %s AND {stock} = %s AND {timestamp} = %s".format(
            ...
            # otherwise get most recent data
                ...
                data = ewutils.execute_sql_query(
                    "SELECT {stock}, {market_rate}, {exchange_rate}, {boombust}, {total_shares}, {timestamp} FROM stocks WHERE id_server = %s AND {stock} = %s ORDER BY {timestamp} DESC".format(
                        stock=ewcfg.col_stock,
                        market_rate=ewcfg.col_market_rate,
                        exchange_rate=ewcfg.col_exchange_rate,
                        boombust=ewcfg.col_boombust,
    ...
    def persist(self):
        ewutils.execute_sql_query(
            "INSERT INTO stocks ({id_server}, {stock}, {market_rate}, {exchange_rate}, {boombust}, {total_shares}, {timestamp}) VALUES(%s, %s, %s, %s, %s, %s, %s)".format(
                id_server=ewcfg.col_id_server,
                stock=ewcfg.col_stock,
                market_rate=ewcfg.col_market_rate,
                exchange_rate=ewcfg.col_exchange_rate,
...
class EwCompany:
        ...
        if id_server is not None and stock is not None:
            ...
            try:
                ...
                result = ewutils.execute_sql_query(
                    "SELECT {recent_profits}, {total_profits} FROM companies WHERE {id_server} = %s AND {stock} = %s".format(
    ...
    def persist(self):
        try:
            ewutils.execute_sql_query(
                "REPLACE INTO companies({recent_profits}, {total_profits}, {id_server}, {stock}) VALUES(%s,%s,%s,%s)".format(
                    recent_profits=ewcfg.col_recent_profits,
                    total_profits=ewcfg.col_total_profits,
                    id_server=ewcfg.col_id_server,
                    stock=ewcfg.col_stock
        ...
        if value != None:
            if stock != None:
                ...
                if user_data.slimecoin < cost_total:
                    ...
                    shares = getUserTotalShares(id_server=user_data.id_server, stock=stock.id_stock,
                                                id_user=user_data.id_user)
                    shares += net_shares
                    updateUserTotalShares(id_server=user_data.id_server, stock=stock.id_stock,
                                          id_user=user_data.id_user, shares=shares)
                    ...
                    response = "You invest {coin:,} SlimeCoin and receive {shares:,} shares in {stock}. Your slimebroker takes his nominal fee of {fee:,} SlimeCoin.".format(
                        coin=value, shares=net_shares, stock=ewcfg.stock_names.get(stock.id_stock),
                ...
                response = "That's not a valid stock name, please use a proper one, you cunt: {}".format(
...
async def withdraw(cmd):
    ...
    if user_data.life_state == ewcfg.life_state_corpse:
        ...
        if stock != None:
            ...
            total_shares = getUserTotalShares(id_server=user_data.id_server, stock=stock.id_stock,
                                              id_user=user_data.id_user)
                ...
                if value <= total_shares:
                    ...
                    if user_data.time_lastinvest + ewcfg.cd_invest > time_now:
                        ...
                        response = "You exchange {shares:,} shares in {stock} for {coins:,} SlimeCoin.".format(
                            coins=slimecoin, shares=shares, stock=ewcfg.stock_names.get(stock.id_stock))
                        user_data.persist()
                        stock.timestamp = round(time.time())
                        stock.persist()
                        updateUserTotalShares(id_server=user_data.id_server, stock=stock.id_stock,
                                              id_user=user_data.id_user, shares=total_shares)
                else:
    ...
    if value != None:
        ...
        if user_data.slimecoin < cost_total:
            ...
            response = "You transfer {slime:,} SlimeCoin to {target_name}. Your slimebroker takes his nominal fee of {fee:,} SlimeCoin.".format(
                slime=value, target_name=member.display_name, fee=(cost_total - value))
    ...
    if user_data.poi != ewcfg.poi_id_stockexchange:
        ...
        if stock in ewcfg.stocks:
            ...
            response = "The current value of {stock} stocks is {cred:,} SlimeCoin per 1000 Shares.".format(
                stock=ewcfg.stock_names.get(stock.id_stock), cred=stock.exchange_rate)
        elif stock == "":
...
def market_tick(stock_data, id_server):
    ...
    exchange_rate_increase = round((market_rate - ewcfg.default_stock_market_rate) * min(stock_data.exchange_rate,
                                                                                         ewcfg.default_stock_exchange_rate) / ewcfg.default_stock_market_rate)
    ...
    if stock_data.exchange_rate < 1000:
        ...
        if stock_data.total_shares > 0:
            ...
            shares = getUserTotalShares(stock=stock_data.id_stock, id_user=majority_shareholder,
                                        id_server=stock_data.id_server)
            shares_lost = int(shares * 0.9)
            stock_data.total_shares -= shares_lost
            updateUserTotalShares(stock=stock_data.id_stock, id_user=majority_shareholder,
                                  id_server=stock_data.id_server, shares=shares - shares_lost)
...
def getRecentTotalShares(id_server=None, stock=None, count=2):
    if id_server != None and stock != None:
        ...
        try:
            ...
            count = round(count)
            data = ewutils.execute_sql_query(
                "SELECT {total_shares} FROM stocks WHERE {id_server} = %s AND {stock} = %s ORDER BY {timestamp} DESC LIMIT %s".format(
                    stock=ewcfg.col_stock,
                    total_shares=ewcfg.col_total_shares,
                    id_server=ewcfg.col_id_server,
                    timestamp=ewcfg.col_timestamp,
...
def getUserTotalShares(id_server=None, stock=None, id_user=None):
    if id_server != None and stock != None and id_user != None:
        ...
        try:
            ...
            data = ewutils.execute_sql_query(
                "SELECT {shares} FROM shares WHERE {id_server} = %s AND {id_user} = %s AND {stock} = %s".format(
                    stock=ewcfg.col_stock,
...
def updateUserTotalShares(id_server=None, stock=None, id_user=None, shares=0):
    if id_server != None and stock != None and id_user != None:
        ...
        try:
            ...
            ewutils.execute_sql_query(
                "REPLACE INTO shares({id_server}, {id_user}, {stock}, {shares}) VALUES(%s, %s, %s, %s)".format(
                    stock=ewcfg.col_stock,
                    shares=ewcfg.col_shares,
                    id_server=ewcfg.col_id_server,
                    id_user=ewcfg.col_id_user
...
def get_user_shares_str(id_server=None, stock=None, id_user=None):
    ...
    if id_server != None and stock != None and id_user != None:
        ...
        response = "You have {shares:,} shares in {stock}".format(shares=shares,
                                                                  stock=ewcfg.stock_names.get(stock.id_stock))
        if user_data.poi == ewcfg.poi_id_stockexchange:
...
def get_majority_shareholder(id_server=None, stock=None):
    ...
    if id_server is not None and stock is not None:
        try:
            ...
            data = ewutils.execute_sql_query(
                "SELECT {id_user}, {shares} FROM shares WHERE {id_server} = %s AND {stock} = %s ORDER BY {shares} DESC LIMIT 1".format(
                    stock=ewcfg.col_stock,
                    shares=ewcfg.col_shares,
                    id_server=ewcfg.col_id_server,
                    id_user=ewcfg.col_id_user
