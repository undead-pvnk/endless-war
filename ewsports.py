...
async def shambleball(cmd):
	...
	district_data = EwDistrict(district = poi_data.id_poi, id_server = cmd.guild.id)
	...
	if game_data == None:
		...
		if game_data == None:
			game_data = EwShambleBallGame(poi_data.id_poi, cmd.guild.id)
		...
		shamble_player = EwShambleBallPlayer(cmd.message.author.id, cmd.guild.id, game_data.id_game, team)
	else:
