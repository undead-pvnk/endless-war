        ...
        for ch in self.channel_responses:
            ...
            try:
                ...
                while len(self.channel_responses[ch]) > 0:
                    if len("{}\n{}".format(response,
                                           self.channel_responses[ch][0])) < ewcfg.discord_message_length_limit:
    ...
    if id_server != None:
        try:
            ...
            for user in users:
                ...
                slimes_to_decay = user_data.slimes - (
                            user_data.slimes * (.5 ** (ewcfg.update_market / ewcfg.slime_half_life)))
            ...
            for district in districts:
                ...
                slimes_to_decay = district_data.slimes - (
                            district_data.slimes * (.5 ** (ewcfg.update_market / ewcfg.slime_half_life)))
            ...
            for user in users:
                ...
                if slimes_to_bleed >= 1:
                    ...
                    if user_data.slimes < 0:
                        ...
                        deathreport = "{skull} *{uname}*: You have succumbed to your wounds. {skull}".format(
                            skull=ewcfg.emote_slimeskull, uname=player_data.display_name)
...
def pushupServerHunger(id_server=None):
    if id_server != None:
        try:
            ...
            # Save data
            cursor.execute(
                "UPDATE users SET {hunger} = {hunger} + {tick} WHERE life_state > 0 AND id_server = %s AND hunger < {limit}".format(
...
def pushdownServerInebriation(id_server=None):
    if id_server != None:
        try:
            ...
            # Save data
            cursor.execute(
                "UPDATE users SET {inebriation} = {inebriation} - {tick} WHERE id_server = %s AND {inebriation} > {limit}".format(
...
def weaponskills_get(id_server=None, id_user=None, member=None):
    ...
    if id_server != None and id_user != None:
        try:
            ...
            cursor.execute(
                "SELECT {weapon}, {weaponskill} FROM weaponskills WHERE {id_server} = %s AND {id_user} = %s".format(
...
def weaponskills_set(id_server=None, id_user=None, member=None, weapon=None, weaponskill=0):
    ...
    if id_server != None and id_user != None and weapon != None:
        try:
            ...
            cursor.execute(
                "REPLACE INTO weaponskills({id_server}, {id_user}, {weapon}, {weaponskill}) VALUES(%s, %s, %s, %s)".format(
                    id_server=ewcfg.col_id_server,
                    id_user=ewcfg.col_id_user,
                    weapon=ewcfg.col_weapon,
                    weaponskill=ewcfg.col_weaponskill
...
def find_kingpin(id_server, kingpin_role):
    data = execute_sql_query(
        "SELECT id_user FROM users WHERE id_server = %s AND {life_state} = %s AND {faction} = %s".format(
