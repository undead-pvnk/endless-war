...
class EwItem:
    ...
    def persist(self):
        try:
            ...
            # Save the object.
            cursor.execute(
                "REPLACE INTO items({}, {}, {}, {}, {}, {}, {}, {}) VALUES(%s, %s, %s, %s, %s, %s, %s, %s)".format(
                    ewcfg.col_id_item,
                    ewcfg.col_id_server,
                    ewcfg.col_id_user,
                    ewcfg.col_item_type,
                    ...
                    self.id_owner,
                    self.item_type,
                    self.time_expir if self.time_expir is not None else self.item_props[
                        'time_expir'] if 'time_expir' in self.item_props.keys() else 0,
                    self.stack_max,
                    self.stack_size,
...
def check_inv_capacity(id_user=None, id_server=None, item_type=None):
    if id_user is not None and id_server is not None and item_type is not None:
        ...
        if item_type == ewcfg.it_food:
            ...
            if len(weapons_held) > math.floor(
                    user_data.slimelevel / ewcfg.max_weapon_mod) if user_data.slimelevel >= ewcfg.max_weapon_mod else len(
                    weapons_held) >= 1:
                return False
            else:
                return True
...
def cmd_is_inventory(cmd):
    return (
                cmd == ewcfg.cmd_inventory or cmd == ewcfg.cmd_inventory_alt1 or cmd == ewcfg.cmd_inventory_alt2 or cmd == ewcfg.cmd_inventory_alt3)
...
async def item_look(cmd):
    ...
    item_sought = find_item(item_search=item_search, id_user=author.id,
                            id_server=server.id if server is not None else None)
...
def find_item(item_search=None, id_user=None, id_server=None):
    ...
    if item_search:
        ...
        for item in items:
            if item.get('id_item') == item_search_int or item_search in ewutils.flattenTokenListToString(
                    item.get('name')):
                item_sought = item
                break
...
async def discard(cmd):
    ...
    item_sought = find_item(item_search=item_search, id_user=cmd.message.author.id,
