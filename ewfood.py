    ...
    if (poi == None) or (len(poi.vendors) == 0):
        ...
        if cmd.tokens_count > 1:
            for token in cmd.tokens[1:]:
                if token.startswith(
                        '<@') == False and token.lower() not in "togo":  # togo can be spelled together or separate
                    value = token
                    break
            for token in cmd.tokens[1:]:
            ...
            if value > user_data.slimecoin:
                ...
                if not togo:
                    ...
                    if target_data != None:
                        ...
                        if target_data.id_user in ewutils.food_multiplier and ewutils.food_multiplier.get(
                                target_data.id_user) > 0:
                            recover_hunger *= ewutils.food_multiplier.get(target_data.id_user)
                            ewutils.food_multiplier[target_data.id_user] += 1
                        else:
                            ewutils.food_multiplier[target_data.id_user] = 1
                        asyncio.ensure_future(ewutils.decrease_food_multiplier(target_data.id_user))
                        target_data.hunger -= recover_hunger
                        ...
                        recover_hunger = food.recover_hunger
                        if user_data.id_user in ewutils.food_multiplier and ewutils.food_multiplier.get(
                                user_data.id_user) > 0:
                            recover_hunger *= ewutils.food_multiplier.get(user_data.id_user)
                            ewutils.food_multiplier[user_data.id_user] += 1
                        else:
                            ewutils.food_multiplier[user_data.id_user] = 1
                        asyncio.ensure_future(ewutils.decrease_food_multiplier(user_data.id_user))
                        user_data.hunger -= recover_hunger
                    ...
                    item_props = {
                        ...
                        'time_expir': time.time() + (
                            food.time_expir if food.time_expir is not None else ewcfg.std_food_expir)
                    }
