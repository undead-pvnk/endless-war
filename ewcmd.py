		...
		if cmd.mentions_count == 1:
			...
			if user_data.poi == target_user_data.poi:
				...
				if user_data.sap < ewcfg.sap_spend_piss:
					...
					user_data.time_expirpvp = ewutils.calculatePvpTimer(user_data.time_expirpvp, (int(time.time()) + ewcfg.time_pvp_attack), enlisted)
					user_data.persist()
...
async def push(cmd):
	...
	if targetmodel.poi != user_data.poi:
		...
		enlisted = True if user_data.life_state == ewcfg.life_state_enlisted else False
		user_data.time_expirpvp = ewutils.calculatePvpTimer(user_data.time_expirpvp, (int(time.time()) + ewcfg.time_pvp_kill), enlisted)
		user_data.persist()
